...
call bbmerge_report {
	...
	output {
		...
		File bbmerge_report_file = bbmerge_report.bbmergefields_table
	}
}
...
task ampseq_bbmerge_process {
	...
	python /Code/Amplicon_TerraPipeline.py --config ~{config_json} --overlap_reads --meta --repo --adaptor_removal --merge
...
task bbmerge_report {
	...
	python /Code/Amplicon_TerraPipeline.py --config ~{config_json} --overlap_reads --bbmerge_report
	...
	runtime {
		...
		preemptible: 3
		maxRetries: 1
		docker: 'jorgeamaya/ci_bbmerge_report:v1'
	}
